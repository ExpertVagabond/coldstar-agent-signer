...
import { TxIntent, TxInstruction, TokenMovement, LAMPORTS_PER_SOL } from "../policy/schema.js";
...
const TOK_DANGEROUS: Record<number, string> = {
 ...
 */
function decodeTokenIx(
  ix: CompiledInstruction,
  feePayer: string,
): { ok: true; value: TxInstruction; movement?: TokenMovement } | { ok: false; reason: string } {
  const data = ix.data;
  if (!data || data.length < 1) return { ok: false, reason: "token instruction has no data" };
  const disc = data[0] as number;
  const self: TxInstruction = { programId: ix.programAddress };

  if (TOK_NON_VALUE.has(disc)) return { ok: true, value: self };
  ...
  if (amount === undefined) return { ok: false, reason: `token instruction ${disc} has no amount field` };
  ...
  if (authority !== feePayer) return { ok: true, value: self };
  ...
  if (mint === undefined) return { ok: false, reason: "TransferChecked is missing its mint account" };
  const decimals = data.length >= 10 ? (data[9] as number) : undefined;
  return {
    ok: true,
    value: self,
...
const STAKE_DANGEROUS: Record<number, string> = {
...
function decodeStakeIx(
  ix: CompiledInstruction,
  feePayer: string,
): { ok: true; value: TxInstruction } | { ok: false; reason: string } {
  const data = ix.data;
  if (!data || data.length < 4) return { ok: false, reason: "stake instruction has no/short data" };
  const disc = readU32LE(data, 0);
  if (disc === undefined) return { ok: false, reason: "stake discriminant unreadable" };

  const danger = STAKE_DANGEROUS[disc];
  if (danger) return { ok: false, reason: `stake instruction ${disc}: ${danger}` };

  // Split moves lamports into a new stake account that inherits our authorities,
  // so nothing leaves our control.
  if (STAKE_NON_VALUE.has(disc) || disc === 3) return { ok: true, value: { programId: ix.programAddress } };

  if (disc !== STAKE_WITHDRAW) return { ok: false, reason: `unknown stake instruction discriminant ${disc}` };

  const lamports = readU64(data, 4);
  if (lamports === undefined) return { ok: false, reason: "stake Withdraw has no lamports field" };
  const accounts = ix.accounts ?? [];
  ...
  if (withdrawAuthority !== feePayer) return { ok: true, value: { programId: ix.programAddress } };
...
interface ComputeBudgetState {
  units?: number;
  microLamports?: bigint;
}

function decodeComputeBudgetIx(
  ix: CompiledInstruction,
  cb: ComputeBudgetState,
): { ok: true; value: TxInstruction } | { ok: false; reason: string } {
  const data = ix.data;
  if (!data || data.length < 1) return { ok: false, reason: "compute budget instruction has no data" };
  const disc = data[0] as number;
  if (disc === CB_SET_UNIT_LIMIT) {
    const units = readU32(data, 1);
    if (units === undefined) return { ok: false, reason: "SetComputeUnitLimit has no units field" };
    cb.units = units;
  } else if (disc === CB_SET_UNIT_PRICE) {
    const price = readU64(data, 1);
    if (price === undefined) return { ok: false, reason: "SetComputeUnitPrice has no price field" };
...
function priorityFeeLamports(cb: ComputeBudgetState, instructionCount: number): bigint {
  if (cb.microLamports === undefined || cb.microLamports === 0n) return 0n;
...
function decodeAtaIx(
  ix: CompiledInstruction,
  feePayer: string,
): { ok: true; value: TxInstruction } | { ok: false; reason: string } {
  // Legacy Create carries no data at all.
  const disc = !ix.data || ix.data.length === 0 ? ATA_CREATE : (ix.data[0] as number);
  ...
  if (payer === undefined) return { ok: false, reason: "associated token account create is missing the payer" };
  if (payer !== feePayer) return { ok: true, value: { programId: ix.programAddress } };
...
const SQUADS_PRIVILEGED: Array<{ disc: number[]; name: string }> = [
...
function discEquals(data: Uint8Array, disc: number[]): boolean {
 ...
 */
function decodeSquadsIx(
  ix: CompiledInstruction,
  feePayer: string,
): { ok: true; value: TxInstruction; movement?: TokenMovement } | { ok: false; reason: string } {
  const data = ix.data;
  if (!data || data.length < 8) return { ok: false, reason: "squads instruction has no discriminator" };
  ...
  if (amount === undefined) return { ok: false, reason: "squads spending_limit_use has no amount field" };
  ...
  if (member !== feePayer) return { ok: true, value: { programId: ix.programAddress } };
  ...
  const isNone = (a: string | undefined) => a === undefined || a === ix.programAddress;
  ...
  if (!isNone(mint)) {
    const decimals = data.length >= 17 ? (data[16] as number) : undefined;
...
export function parseTx(message: DecompiledMessage): ParseResult {
  ...
  const tokenMovements: TokenMovement[] = [];
  const computeBudget: ComputeBudgetState = {};
  let outLamports = 0n;

  for (const ix of message.instructions) {
    const decoded: { ok: true; value: TxInstruction; movement?: TokenMovement } | { ok: false; reason: string } =
            ...
            : ix.programAddress === COMPUTE_BUDGET_PROGRAM_ID
              ...
              : ix.programAddress === STAKE_PROGRAM_ID
                ...
                : ix.programAddress === MEMO_PROGRAM_ID || ix.programAddress === MEMO_LEGACY_PROGRAM_ID
                  ? { ok: true as const, value: { programId: ix.programAddress } }
    ...
    if (decoded.movement) tokenMovements.push(decoded.movement);
  }

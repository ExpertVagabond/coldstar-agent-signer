...
if (expiresArg) {
  ...
  if (m) {
    ...
    if (Number.isNaN(expiresAt.getTime())) fail(`--expires: cannot parse '${expiresArg}' (use 24h, 7d, or ISO-8601)`);
  }
}
...
if (isEncryptedKeyContainer(rootRaw)) {
  ...
  root = Keypair.fromSecretKey(Uint8Array.from(rootRaw as number[]));
} else {

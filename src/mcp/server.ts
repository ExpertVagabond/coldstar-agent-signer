...
function deserialize(base64Tx: string): VersionedTransaction {
...
type OutcomeT = z.infer<z.ZodObject<typeof Outcome>>;
 ...
 */
async function broadcast(rpcUrl: string, cluster: string, out: OutcomeT): Promise<OutcomeT> {
  try {
    const connection = new Connection(rpcUrl, "confirmed");
    const signature = await connection.sendRawTransaction(b64.decode(out.signed_transaction_base64 as string));
...
export function createColdstarMcpServer(opts: McpServerOptions): McpServer {
  ...
  server.registerTool(
    "coldstar_status",
    {
      ...
      description:
        "The agent's session wallet address, the policy it is bound by, and how much of today's daily cap is used. The root key is never available here.",
      inputSchema: {},
      outputSchema: {
  ...
  server.registerTool(
    "coldstar_verdict",
    {
      ...
      description:
        "Evaluate a base64-encoded Solana transaction against the wallet policy and return AUTO_SIGN, ESCALATE, or REJECT with the reason. Nothing is signed. Use this to pre-flight a plan.",
  ...
  server.registerTool(
    "coldstar_sign",
    {
      ...
      description:
        "Sign a base64 Solana transaction with the session key if policy allows. Returns status 'signed' with the signed transaction, 'escalated' with the unsigned transaction for a human to approve on the air-gapped device, or 'rejected' with no signature. Does not broadcast.",
      inputSchema: { transaction_base64: z.string().describe("Unsigned transaction, base64 (legacy or v0)") },
  ...
  server.registerTool(
    "coldstar_sign_and_send",
    {
      ...
      description:
        "Like coldstar_sign, then broadcasts a signed transaction to the configured RPC and returns the signature. Escalated and rejected outcomes are returned, not broadcast. A broadcast failure returns status send_failed with the RPC reason.",
      inputSchema: { transaction_base64: z.string().describe("Unsigned transaction, base64 (legacy or v0)") },
      outputSchema: Outcome,
    },
    async ({ transaction_base64 }) => {
      const out = await signOrExplain(deserialize(transaction_base64));
      if (out.status !== "signed") return text(out);
  ...
  server.registerTool(
    "coldstar_transfer_sol",
    {
      ...
      description:
        "Build a SOL transfer from the session wallet, evaluate it against policy, and broadcast if allowed. Amounts over the escalate threshold or to non-allowlisted recipients are escalated; blocklisted recipients are rejected with no signature.",
      inputSchema: {

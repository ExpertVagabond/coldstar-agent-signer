 ...
 */
export function activeInterfaces(): ActiveInterface[] {
  const out: ActiveInterface[] = [];
...
const CAVEAT =
  "No active interface is not proof of isolation: a VM on a networked host, a tether attached later, " +
  "Thunderbolt networking, or a radio the OS does not enumerate would not appear here.";

export function checkAirGap(): AirGapCheck {
 ...
 */
export function describeAirGap(check: AirGapCheck): string {
  if (check.airGapped) return `no active network interfaces. ${check.caveat}`;

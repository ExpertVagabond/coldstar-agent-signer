 ...
 */
export function isLegacyKeyContainer(v: unknown): v is LegacyKeyContainer {
  if (!v || typeof v !== "object" || Array.isArray(v)) return false;
  const c = v as Record<string, unknown>;
  if (typeof c.salt !== "string" || typeof c.nonce !== "string" || typeof c.ciphertext !== "string") {
    return false;
  }
  if (typeof c.algo === "string") return c.algo.endsWith("_xsalsa20poly1305");
  // No `algo`: fall back to shape. Hex, and libsodium's sizes.
  if (!/^[0-9a-fA-F]+$/.test(c.salt) || !/^[0-9a-fA-F]+$/.test(c.nonce)) return false;
  return c.salt.length === LIBSODIUM_SALT_SIZE * 2 && c.nonce.length === SECRETBOX_NONCE_SIZE * 2;
}

function fromHex(s: string, what: string): Uint8Array {
 ...
 * different key.
 */
function deriveLegacyKey(passphrase: string, salt: Uint8Array, container: LegacyKeyContainer): Uint8Array {
 ...
 */
export function decryptLegacyRootKey(container: LegacyKeyContainer, passphrase: string): Uint8Array {
  if (!isLegacyKeyContainer(container)) throw new Error("not a legacy key container");
  ...
  if (sealed.length <= SECRETBOX_TAG_SIZE) throw new Error("ciphertext is too short to contain a tag");

...
One limit on that sentence, because it is the kind a vendor is tempted to leave out, and it is a place where this package is weaker than Coldstar itself. Coldstar's Rust signer decrypts into a buffer it locks with `mlock` and zeroizes on drop (`secure_signer/src/secure_buffer.rs`, used on every plaintext-key path in `crypto.rs`). This package does not: `coldstar-sign-policy` and the Python tool decrypt into an ordinary JavaScript or Python heap, nothing locks those pages against swap, and zeroing a buffer there does not reach copies the runtime may already have made. What protects the key in use is that the machine doing the decryption is not reachable, not the wiping. See `COLDSTAR-PARITY.md`.

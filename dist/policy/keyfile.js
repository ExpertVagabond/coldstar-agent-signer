 ...
 */
export function encryptRootKey(secret, passphrase, publicKeyBase58) {
 ...
 */
export function decryptRootKey(container, passphrase) {
    if (!isEncryptedKeyContainer(container))
        throw new Error("not an encrypted key container");
    if (container.version !== 1)
        throw new Error(`unsupported key container version ${container.version}`);
    ...
    try {
        ...
        if (seed.length !== SEED_SIZE)
            throw new Error(`decrypted key is ${seed.length} bytes, expected ${SEED_SIZE}`);
...
export function containerMatchesPublicKey(container, publicKey) {
    if (!container.public_key)

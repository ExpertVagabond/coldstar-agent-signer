...
export function revocationMemo(sessionPubkey) {
...
export class RevocationChecker {
    ...
    async status() {
        if (this.last?.state === "revoked")
            return this.last;
        if (this.last?.state === "active" && this.now() - this.last.checkedAt < this.freshnessMs)
            return this.last;
    ...
    async check() {
        ...
        try {
            ...
            return { state: "unknown", reason: e.message ?? String(e), checkedAt: at };
        }
    }
}
